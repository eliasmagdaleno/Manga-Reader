...
///       -only-testing:Manga-ReaderTests/TagPairSeedingDiagnostic
///
...
final class TagPairSeedingDiagnostic: XCTestCase {
    ...
    private static func preferences(in base: URL) throws -> [String: Any] {
        ...
        for url in plists where url.lastPathComponent.contains("Manga-Reader") {
            if let dict = NSDictionary(contentsOf: url) as? [String: Any] { return dict }
        }
        throw XCTSkip("no Manga-Reader preferences plist under \(dir.path) — found \(plists.map(\.lastPathComponent))")
    }
}
