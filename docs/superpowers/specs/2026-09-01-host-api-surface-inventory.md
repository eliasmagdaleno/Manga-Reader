...
(`Manga-Reader/Models/WeebCentralSource.swift:29-34`,
`Manga-Reader/Services/SourceContext.swift:16-22`). Every network-backed operation calls the same
...
(`Manga-Reader/Services/SourceContext.swift:13-18`). This generic/metatype shape describes the
...
|---|---|---|---|
| `search(title:limit:offset:)` | Builds `/search/data` with `sort=Best Match`, `display_mode=Full Display`, decimal `limit`/`offset`, and a nonempty `text`; then uses the series-list script (`Manga-Reader/Models/WeebCentralSource.swift:38-40`, `Manga-Reader/Models/WeebCentralSource.swift:108-121`). | Expects an array of `{id: String, title: String, cover?: String}`. The script drops rows missing an id or title and resolves relative covers against the loaded page; Swift maps each remaining row to a Listing-shaped `Manga`, with empty description, `unknown` status, no year or MAL id, and a cover only when its string forms a `URL` (`Manga-Reader/Models/WeebCentralSource.swift:96-105`, `Manga-Reader/Models/WeebCentralSource.swift:134-138`, `Manga-Reader/Models/WeebCentralSource.swift:168-188`). | URL construction is non-failable because it starts from a constant valid base. Extraction errors propagate unchanged. A malformed optional cover is silently converted to `nil`; malformed/missing rows are silently filtered by the script (`Manga-Reader/Models/WeebCentralSource.swift:38-40`, `Manga-Reader/Models/WeebCentralSource.swift:96-105`, `Manga-Reader/Models/WeebCentralSource.swift:175-186`). |
| `popular(limit:offset:)` | Uses the same `/search/data` path and script with no `text` and `sort=Popularity` (`Manga-Reader/Models/WeebCentralSource.swift:42-44`, `Manga-Reader/Models/WeebCentralSource.swift:108-121`). | Same series-list wire shape and `Manga` mapping as search (`Manga-Reader/Models/WeebCentralSource.swift:96-105`, `Manga-Reader/Models/WeebCentralSource.swift:134-138`). | Same propagation and silent row/cover loss as search (`Manga-Reader/Models/WeebCentralSource.swift:96-105`, `Manga-Reader/Models/WeebCentralSource.swift:175-186`). |
| `newTitles(limit:offset:)` | Uses the same `/search/data` path and script with no `text` and `sort=Recently Added` (`Manga-Reader/Models/WeebCentralSource.swift:46-48`, `Manga-Reader/Models/WeebCentralSource.swift:108-121`). | Same series-list wire shape and `Manga` mapping as search (`Manga-Reader/Models/WeebCentralSource.swift:96-105`, `Manga-Reader/Models/WeebCentralSource.swift:134-138`). | Same propagation and silent row/cover loss as search (`Manga-Reader/Models/WeebCentralSource.swift:96-105`, `Manga-Reader/Models/WeebCentralSource.swift:175-186`). |
| `latestUpdates(limitTitles:language:offset:)` | Ignores `language`, converts the item offset to a 1-based page as `offset / limitTitles + 1` (or page 1 when the limit is not positive), loads `/latest-updates/{page}`, and runs the updates script (`Manga-Reader/Models/WeebCentralSource.swift:50-56`). | Expects an array of `{mangaId: String, chapterId: String, title: String, cover?: String}`. The script drops entries missing either id or title and resolves relative covers. Swift truncates to `limitTitles` and maps each entry to `MangaUpdate`; the embedded `Manga` uses the same sparse mapping as list results (`Manga-Reader/Models/WeebCentralSource.swift:57-59`, `Manga-Reader/Models/WeebCentralSource.swift:103-105`, `Manga-Reader/Models/WeebCentralSource.swift:153-158`, `Manga-Reader/Models/WeebCentralSource.swift:250-271`). | Extraction errors propagate. Invalid optional covers become `nil`; incomplete DOM entries are silently filtered. A negative `limitTitles` reaches `prefix(_:)`, whose precondition is nonnegative, because this method performs no input validation (`Manga-Reader/Models/WeebCentralSource.swift:50-59`, `Manga-Reader/Models/WeebCentralSource.swift:257-270`). |
| `mangaDetail(id:)` | Loads `/series/{id}` and runs the detail script (`Manga-Reader/Models/WeebCentralSource.swift:62-65`). | Expects `{description?: String, authors: [String], tags: [String], adult?: Bool}`. DOM absence becomes an empty/nil value in the script. Swift turns tag names into `Tag` values with empty id/group, defaults description to `""`, and maps `adult == true` to `erotica`, everything else to `safe` (`Manga-Reader/Models/WeebCentralSource.swift:65-71`, `Manga-Reader/Models/WeebCentralSource.swift:140-145`, `Manga-Reader/Models/WeebCentralSource.swift:190-216`). | Extraction errors propagate. Missing adult evidence is treated as safe rather than unknown; missing description becomes empty. Missing required `authors` or `tags` keys is a decode failure because those DTO fields are non-optional (`Manga-Reader/Models/WeebCentralSource.swift:65-71`, `Manga-Reader/Models/WeebCentralSource.swift:140-145`). |
| `chapters(mangaId:)` | Loads `/series/{mangaId}/full-chapter-list` and runs the chapter script (`Manga-Reader/Models/WeebCentralSource.swift:74-77`). | Expects an array of `{id: String, title: String, date?: String}`; the script drops only rows without an id. Swift derives the displayed number from the last numeric token (or `?`), retains the title, and parses fractional or plain ISO-8601 dates, returning `nil` for missing/empty/invalid dates (`Manga-Reader/Models/WeebCentralSource.swift:78-79`, `Manga-Reader/Models/WeebCentralSource.swift:124-129`, `Manga-Reader/Models/WeebCentralSource.swift:147-151`, `Manga-Reader/Models/WeebCentralSource.swift:218-238`, `Manga-Reader/Models/MangaDexAPI.swift:204-214`). | Extraction errors propagate. Missing `title` causes decoding to fail even though the script filters only on id. An unparseable title/date degrades to `?`/`nil` rather than throwing (`Manga-Reader/Models/WeebCentralSource.swift:78-79`, `Manga-Reader/Models/WeebCentralSource.swift:124-129`, `Manga-Reader/Models/WeebCentralSource.swift:147-151`, `Manga-Reader/Models/WeebCentralSource.swift:225-236`). |
| `pageURLs(chapterId:preferDataSaver:)` | Ignores `preferDataSaver`, loads `/chapters/{chapterId}/images?reading_style=long_strip`, and runs the pages script (`Manga-Reader/Models/WeebCentralSource.swift:82-86`). | Expects `[String]`. The script reads `src`/`data-src`, drops absent attributes, and resolves relative values against the page; Swift then drops any string that cannot initialize a `URL` (`Manga-Reader/Models/WeebCentralSource.swift:86-87`, `Manga-Reader/Models/WeebCentralSource.swift:240-248`). | Extraction errors propagate. Missing DOM images and malformed returned URLs are silent omissions, including the possibility of a successful empty page list (`Manga-Reader/Models/WeebCentralSource.swift:86-87`, `Manga-Reader/Models/WeebCentralSource.swift:240-247`). |

`webURL(forManga:)` does not consume a host capability: it synchronously appends
`series/{id}` to the compiled-in base URL (`Manga-Reader/Models/WeebCentralSource.swift:90-92`).
...
(`Manga-Reader/Models/WeebCentralSource.swift:94-167`).
...
(`Manga-Reader/Services/WebViewService.swift:94-114`,
`Manga-Reader/Services/WebViewService.swift:130-159`,
`Manga-Reader/Services/WebViewService.swift:161-188`,
`Manga-Reader/Services/WebViewService.swift:240-250`). Navigation has a 30-second timeout;
interactive challenges have a 120-second timeout; scripts have a 15-second timeout
(`Manga-Reader/Services/WebViewService.swift:31-35`,
`Manga-Reader/Services/WebViewService.swift:130-170`).
...
(`Manga-Reader/Services/WebViewService.swift:85-91`,
`Manga-Reader/Services/WebViewService.swift:130-188`,
`Manga-Reader/Services/WebViewService.swift:230-238`).
...
domain type (`Manga-Reader/Models/MangaSource.swift:9-12`). The actual surface is:

| Member | Shape | Required behavior today |
|---|---|---|
| Identity | `id: String`, `name: String` | Required; no defaults (`Manga-Reader/Models/MangaSource.swift:18-22`). |
| Adult declaration | `isNSFW: Bool` | Optional behavior through a default of `false` (`Manga-Reader/Models/MangaSource.swift:23-24`, `Manga-Reader/Models/MangaSource.swift:102-104`). |
| Search | `search(title: String, limit: Int, offset: Int) async throws -> [Manga]` | Required; no default (`Manga-Reader/Models/MangaSource.swift:26-27`). |
| Tag browsing declaration | `supportsTagBrowse: Bool` | Optional behavior through a default of `false` (`Manga-Reader/Models/MangaSource.swift:28-32`, `Manga-Reader/Models/MangaSource.swift:105-105`). |
| Tag browsing | `mangaByTag(tag: String, limit: Int, offset: Int) async throws -> [Manga]` | Optional; default throws `SourceError.unsupported("mangaByTag")` (`Manga-Reader/Models/MangaSource.swift:33-35`, `Manga-Reader/Models/MangaSource.swift:107-109`). |
| Popular feed | `popular(limit: Int, offset: Int) async throws -> [Manga]` | Required; no default (`Manga-Reader/Models/MangaSource.swift:36-37`). |
| New-title feed | `newTitles(limit: Int, offset: Int) async throws -> [Manga]` | Optional; default throws `SourceError.unsupported("newTitles")` (`Manga-Reader/Models/MangaSource.swift:38-39`, `Manga-Reader/Models/MangaSource.swift:111-113`). |
| Latest-update feed | `latestUpdates(limitTitles: Int, language: String, offset: Int) async throws -> [MangaUpdate]` | Optional; default throws `SourceError.unsupported("latestUpdates")`. A Swift-only convenience overload supplies `offset: 0` (`Manga-Reader/Models/MangaSource.swift:40-45`, `Manga-Reader/Models/MangaSource.swift:115-123`). |
| Detail | `mangaDetail(id: String) async throws -> MangaDetail` | Required; no default (`Manga-Reader/Models/MangaSource.swift:46-47`). |
| Chapters | `chapters(mangaId: String) async throws -> [Chapter]` | Required; no default (`Manga-Reader/Models/MangaSource.swift:48-49`). |
| Pages | `pageURLs(chapterId: String, preferDataSaver: Bool) async throws -> [URL]` | Required; no default (`Manga-Reader/Models/MangaSource.swift:50-51`). |
| Image concurrency | `imagePrefetchConcurrency: Int` | Optional behavior through a default of 5 (`Manga-Reader/Models/MangaSource.swift:52-54`, `Manga-Reader/Models/MangaSource.swift:125-125`). |
| Human-facing site URL | `webURL(forManga id: String) -> URL?` | Optional behavior through a default of `nil` (`Manga-Reader/Models/MangaSource.swift:55-57`, `Manga-Reader/Models/MangaSource.swift:127-127`). |
| Home presentation | `homeRailTitles: [String]`, `homeRailEyebrows: [String]`, `latestRailShowsNewBadge: Bool` | Optional behavior through defaults of three titles, no eyebrows, and `true`; they are protocol requirements so existential dispatch reaches source overrides (`Manga-Reader/Models/MangaSource.swift:59-69`, `Manga-Reader/Models/MangaSource.swift:129-132`). |
...
- `pageURLs` takes a `Bool` and returns Foundation `[URL]`; `isNSFW`, `supportsTagBrowse`, and
  ...
  (`Manga-Reader/Models/MangaSource.swift:9-12`, `Manga-Reader/Models/MangaSource.swift:23-24`,
  `Manga-Reader/Models/MangaSource.swift:28-32`, `Manga-Reader/Models/MangaSource.swift:50-51`,
  `Manga-Reader/Models/MangaSource.swift:68-69`).
- Of the domain returns, only `Manga` is `Codable`. `MangaUpdate`, `Chapter`, and `MangaDetail`
  ...
  (`Manga-Reader/Models/MangaDexAPI.swift:18-42`,
  `Manga-Reader/Models/MangaDexAPI.swift:44-55`,
  `Manga-Reader/Models/MangaDexAPI.swift:190-215`,
  `Manga-Reader/Models/MangaDexAPI.swift:225-238`).
- The protocol's `throws` boundary does not constrain failures to `SourceError`. MangaDex forwards
  ...
  (`Manga-Reader/Models/MangaDexSource.swift:26-55`,
  `Manga-Reader/Models/MangaDexAPI.swift:401-436`,
  `Manga-Reader/Services/WebViewService.swift:85-91`).
- Capability support has two different representations: explicit boolean plus throwing method for
  ...
  (`Manga-Reader/Models/MangaSource.swift:97-132`).
- The latest-updates offset describes the underlying source feed rather than a guaranteed offset
  ...
  semantics (`Manga-Reader/Models/MangaSource.swift:40-45`,
  `Manga-Reader/Models/WeebCentralSource.swift:50-59`,
  `Manga-Reader/Models/MangaDexAPI.swift:506-557`).
...
logging, clock, or other member (`Manga-Reader/Services/SourceContext.swift:13-22`). The registry
...
(`Manga-Reader/Services/SourceRegistry.swift:54-60`,
`Manga-Reader/Models/MangaDexSource.swift:13-55`).
...
- One shared off-screen `WKWebView` uses `WKWebsiteDataStore.default()`, so browser data including
  ...
  (`Manga-Reader/Services/WebViewService.swift:68-77`).
- The script must evaluate to a Swift `String` containing JSON. A JavaScript object/array, `null`, or
  ...
  (`Manga-Reader/Services/WebViewService.swift:173-186`,
  `Manga-Reader/Models/WeebCentralSource.swift:160-165`).
- Extracts are single-file across every WebView-backed Source: one load/challenge/script pipeline
  owns the shared browser at a time (`Manga-Reader/Services/WebViewService.swift:94-115`,
  `Manga-Reader/Services/WebViewService.swift:240-250`).
- A main-frame response whose `cf-mitigated` header is `challenge` triggers a wait. The service
  ...
  `cancelChallenge()` (`Manga-Reader/Services/WebViewService.swift:23-29`,
  `Manga-Reader/Services/WebViewService.swift:256-285`,
  `Manga-Reader/ContentView.swift:98-100`).
- A challenge can resolve through a challenge-free navigation, explicit dismissal, or the
  ...
  not blocked by it (`Manga-Reader/Services/WebViewService.swift:47-54`,
  `Manga-Reader/Services/WebViewService.swift:101-112`,
  `Manga-Reader/Services/WebViewService.swift:147-159`,
  `Manga-Reader/Services/WebViewService.swift:205-219`).
- Cancellation is checked after a queued extract obtains the WebView lock. After navigation it is
  ...
  stale failures (`Manga-Reader/Services/WebViewService.swift:94-113`,
  `Manga-Reader/Services/WebViewService.swift:130-188`).
- `runScript` times out after 15 seconds. Each run increments `scriptGeneration`; the completion
  ...
  later extraction's continuation (`Manga-Reader/Services/WebViewService.swift:31-46`,
  `Manga-Reader/Services/WebViewService.swift:161-188`).
- If WebKit terminates the content process, the service fails pending load, challenge, and script
  ...
  (`Manga-Reader/Services/WebViewService.swift:298-316`).
...
|---|---|
| `isNSFW` | Filters the source picker, controls whether adult settings are relevant, affects background-refresh presentation, and suppresses notification detail when adult content is hidden (`Manga-Reader/Services/SourceRegistry.swift:85-102`, `Manga-Reader/Services/LibraryRefreshCoordinator.swift:177-180`, `Manga-Reader/Services/UpdateNotifier.swift:133-140`). WeebCentral explicitly declares `false`; MangaDex inherits the default (`Manga-Reader/Models/WeebCentralSource.swift:21-23`, `Manga-Reader/Models/MangaSource.swift:102-104`). |
| `supportsTagBrowse` | Controls whether detail tags can become browse links. MangaDex opts in; WeebCentral inherits `false` and does not implement `mangaByTag` (`Manga-Reader/Views/MangaDetailView.swift:133-139`, `Manga-Reader/Models/MangaDexSource.swift:24-31`, `Manga-Reader/Models/WeebCentralSource.swift:36-92`). |
| `imagePrefetchConcurrency` | Passed to the image cache after page URLs load. Both sources currently inherit 5, even though the hook exists for rate-sensitive image CDNs (`Manga-Reader/Models/ReaderViewModel.swift:160-167`, `Manga-Reader/Models/MangaSource.swift:52-54`, `Manga-Reader/Models/MangaSource.swift:125-125`). |
| `webURL(forManga:)` | Supplies the detail screen's “open in browser” destination. Both built-ins construct source-specific URLs; the default is `nil` (`Manga-Reader/Views/MangaDetailView.swift:37-41`, `Manga-Reader/Models/WeebCentralSource.swift:90-92`, `Manga-Reader/Models/MangaDexSource.swift:58-60`, `Manga-Reader/Models/MangaSource.swift:127-127`). |
| Home rail titles, eyebrows, and latest-update badge | `HomeView` expects three titles (falling back if fewer are supplied), indexes up to three eyebrows, and uses the badge flag to change the middle rail's stamp and tint. Both built-ins override only eyebrows; the remaining values come from defaults (`Manga-Reader/Views/HomeView.swift:146-161`, `Manga-Reader/Models/MangaSource.swift:63-69`, `Manga-Reader/Models/MangaSource.swift:129-132`, `Manga-Reader/Models/WeebCentralSource.swift:25-27`, `Manga-Reader/Models/MangaDexSource.swift:21-24`). |
| Stable source id stamping | WeebCentral's mapping explicitly stamps every returned `Manga` with `weebcentral`; MangaDex stamps during API-to-domain conversion. The registry later resolves a Source from that persisted `sourceId` (`Manga-Reader/Models/WeebCentralSource.swift:18-21`, `Manga-Reader/Models/WeebCentralSource.swift:102-105`, `Manga-Reader/Models/MangaDexAPI.swift:129-140`, `Manga-Reader/Services/SourceRegistry.swift:67-82`). |
...
- WeebCentral owns a base URL, URL/query construction, DOM scripts, parsing helpers, and private
  decode DTOs in compiled Swift (`Manga-Reader/Models/WeebCentralSource.swift:29-34`,
  `Manga-Reader/Models/WeebCentralSource.swift:94-167`).
- MangaDex owns a plain-HTTP client with a custom user agent, status validation, JSON decoding,
  ...
  (`Manga-Reader/Models/MangaDexAPI.swift:390-436`,
  `Manga-Reader/Services/SourceContext.swift:20-22`).
- MangaDex supports tag browsing, preserves alternate titles and a MAL id on Listings, returns
  ...
  the corresponding site data (`Manga-Reader/Models/MangaDexSource.swift:24-55`,
  `Manga-Reader/Models/MangaDexAPI.swift:119-140`,
  `Manga-Reader/Models/MangaDexAPI.swift:225-260`,
  `Manga-Reader/Models/MangaDexAPI.swift:582-626`,
  `Manga-Reader/Models/WeebCentralSource.swift:50-105`). This is inventory only; the reason
...
1. **What is the wire representation and validation policy for every domain result?** The protocol
   ...
   before returning domain values (`Manga-Reader/Models/MangaSource.swift:26-57`,
   `Manga-Reader/Models/MangaDexAPI.swift:18-55`,
   `Manga-Reader/Models/MangaDexAPI.swift:190-238`,
   `Manga-Reader/Models/WeebCentralSource.swift:94-158`). Which fields are required, optional,
...
2. **How does an Extension declare supported capabilities?** Today tag browse uses a boolean plus an
   ...
   (`Manga-Reader/Models/MangaSource.swift:26-69`,
   `Manga-Reader/Models/MangaSource.swift:97-132`). Is support declared before invocation, inferred
...
3. **Which behavior belongs in a manifest and which belongs in executable entry points?** Stable id,
   ...
   rest mix defaults with overrides (`Manga-Reader/Models/MangaSource.swift:18-69`,
   `Manga-Reader/Models/MangaSource.swift:97-132`). What must be knowable without running extension
   code?
4. **What plain-HTTP capability is exposed, and what policy does the host enforce?** The chosen
   ...
   decoding, and one `Retry-After`-aware retry (`Manga-Reader/Services/SourceContext.swift:13-22`,
   `Manga-Reader/Models/MangaDexAPI.swift:401-436`). Who controls method, headers, body, cookies,
...
5. **What WebView primitive crosses into JavaScriptCore?** Swift currently hands
   ...
   (`Manga-Reader/Services/SourceContext.swift:13-18`,
   `Manga-Reader/Services/WebViewService.swift:85-91`,
   `Manga-Reader/Services/WebViewService.swift:173-186`). Does an Extension receive raw JSON, parsed
...
6. **What concurrency and cancellation guarantees are contractual?** The current service globally
   ...
   (`Manga-Reader/Services/WebViewService.swift:94-170`,
   `Manga-Reader/Services/WebViewService.swift:240-250`,
   `Manga-Reader/Models/MangaSource.swift:26-51`). Can calls overlap, can a Source hold work after its
...
7. **What error taxonomy and observability cross the boundary?** `SourceError` has unsupported,
   ...
   (`Manga-Reader/Models/MangaSource.swift:72-95`,
   `Manga-Reader/Models/MangaDexAPI.swift:401-436`,
   `Manga-Reader/Models/WeebCentralSource.swift:96-105`,
   `Manga-Reader/Models/WeebCentralSource.swift:240-247`). Which failures are retriable, user-facing,
...
8. **What rate-limiting and resource-budget controls exist per Extension, per origin, and globally?**
   ...
   prefetch has a per-Source width (`Manga-Reader/Models/MangaDexAPI.swift:406-436`,
   `Manga-Reader/Services/WebViewService.swift:240-250`,
   `Manga-Reader/Models/MangaSource.swift:52-54`). How are request rates, concurrent HTTP work,
...
9. **What storage scopes and lifecycle are available?** `SourceContext` has no storage member, while
   ...
   source id in global `UserDefaults` (`Manga-Reader/Services/SourceContext.swift:20-22`,
   `Manga-Reader/Services/WebViewService.swift:68-75`,
   `Manga-Reader/Services/SourceRegistry.swift:22-27`). Are Extension key/value data, cookies,
...
10. **How are API and Extension versions negotiated over time?** `MangaSource` and
    ...
    defaults to preserve existing Swift conformers (`Manga-Reader/Models/MangaSource.swift:18-70`,
    `Manga-Reader/Models/MangaSource.swift:97-132`,
    `Manga-Reader/Services/SourceContext.swift:20-22`). What version is declared, what compatibility
...
11. **What are the pagination and input-validity contracts?** Most feeds accept `limit` and
    ...
    (`Manga-Reader/Models/MangaSource.swift:26-45`,
    `Manga-Reader/Models/WeebCentralSource.swift:50-59`,
    `Manga-Reader/Models/MangaDexAPI.swift:506-557`). Who validates inputs, and how does a caller know
...
12. **Which Source-authored presentation is stable API, and how is it validated/localized?** Home
    ...
    while eyebrow access is conditional by index (`Manga-Reader/Views/HomeView.swift:146-161`,
    `Manga-Reader/Models/MangaSource.swift:63-69`). Are those labels Extension-authored data, host
...
13. **How are browser identity, cookies, and interactive challenges isolated and disclosed?** All
    ...
    challenge sheet (`Manga-Reader/Services/WebViewService.swift:21-29`,
    `Manga-Reader/Services/WebViewService.swift:68-77`,
    `Manga-Reader/Services/SourceRegistry.swift:54-60`,
    `Manga-Reader/ContentView.swift:98-100`). Are cookies shared across Extensions, may authors alter
...
14. **Which URL schemes and destinations may an Extension produce?** Listings contain optional
    ...
    (`Manga-Reader/Models/MangaDexAPI.swift:18-26`,
    `Manga-Reader/Models/MangaSource.swift:50-57`,
    `Manga-Reader/Models/WeebCentralSource.swift:82-92`,
    `Manga-Reader/Models/WeebCentralSource.swift:96-105`). Which schemes, origins, redirects, and
...
15. **What language semantics apply to feeds and chapter lists?** `latestUpdates` accepts a language
    ...
    (`Manga-Reader/Models/MangaSource.swift:40-49`,
    `Manga-Reader/Models/MangaDexAPI.swift:517-529`,
    `Manga-Reader/Models/MangaDexAPI.swift:582-596`,
    `Manga-Reader/Models/WeebCentralSource.swift:50-53`). Is language a per-call input, Source
...
16. **What happens when a background Source call encounters an interactive challenge?** Library
    ...
    (`Manga-Reader/Services/LibraryRefreshCoordinator.swift:166-186`,
    `Manga-Reader/Services/WebViewService.swift:23-29`,
    `Manga-Reader/Services/WebViewService.swift:94-115`,
    `Manga-Reader/Services/WebViewService.swift:147-159`,
    `Manga-Reader/ContentView.swift:98-100`). May a non-foreground operation request interaction,
...
18. **How is an Extension's adult-content declaration established and trusted?** `isNSFW` controls
    ...
    (`Manga-Reader/Models/MangaSource.swift:23-24`,
    `Manga-Reader/Models/MangaSource.swift:102-104`,
    `Manga-Reader/Services/SourceRegistry.swift:85-102`,
    `Manga-Reader/Services/LibraryRefreshCoordinator.swift:166-181`,
    `Manga-Reader/Services/UpdateNotifier.swift:133-140`). Is the declaration author-supplied,
