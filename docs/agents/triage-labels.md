...
`good first issue`, `help wanted`, `invalid`, `question`), so `/triage` will create them on first
use. No overrides are needed.

**Triage is for issues you did not create.** Tickets produced by `/to-tickets` are already
agent-ready and should not be run through it.
